...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (estructuras && estructuras.length > 0) {
      type EstRow = { km: number | null; geom: unknown };
      let e1: EstRow | null = null;
      let e2: EstRow | null = null;

      for (const est of estructuras) {
        const estKm = Number(est.km);
          if (Number.isFinite(estKm) && estKm <= km) e1 = est as EstRow;
          if (Number.isFinite(estKm) && estKm >= km && !e2) e2 = est as EstRow;
      }
