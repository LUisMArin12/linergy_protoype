...
function getExtendedDataValue(extendedData: unknown, key: string): string | null {
  const ed = extendedData as Record<string, unknown> | null;
  if (!ed || !ed.Data) return null;

  const dataArray = ensureArray(ed.Data as unknown as Record<string, unknown>[]);
  for (const data of dataArray) {
    const name = (data as Record<string, unknown>)["@_name"];
...
async function processFolderStructure(
  ...
  const folders = ensureArray((document.Folder as unknown) as Record<string, unknown>[]);

  for (const folder of folders) {
    const folderName = (folder as Record<string, unknown>).name;
    ...
    const subFolders = ensureArray(((folder as Record<string, unknown>).Folder as unknown) as Record<string, unknown>[]);
    let lineaAereaFolder: Record<string, unknown> | null = null;
    let estructurasFolder: Record<string, unknown> | null = null;
...
async function processPlacemarkStructure(
  ...
  type Placemark = Record<string, unknown> & {
    ExtendedData?: unknown;
    LineString?: unknown;
    Point?: unknown;
  };

  const placemarks = ensureArray((document.Placemark as unknown) as Placemark[]);

  const lineasMap = new Map<string, { tramos: Placemark[]; estructuras: Placemark[] }>();
  ...
  for (const placemark of placemarks) {
    ...
    const isSubestacion = getExtendedDataValue(placemark.ExtendedData, "subestacion");

    if (placemark.LineString) {
      lineaData.tramos.push(placemark);
    } else if (placemark.Point && (isEstructura || isSubestacion)) {
