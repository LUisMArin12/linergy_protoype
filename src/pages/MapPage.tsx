...
import { supabase, Linea, Estructura, Falla, Subestacion } from '../lib/supabase';
import type { Classification, FaultStatus } from '../types';
...
export default function MapPage() {
  ...
  const { data: allSubestaciones = [] } = useQuery({
    queryKey: ['subestaciones'],
    queryFn: async () => {
      const { data, error } = await supabase.rpc('get_subestaciones_geojson');
      if (error) throw error;
      return (data || []) as Subestacion[];
    },
  });
  ...
  const lineas = useMemo(() => {
    ...
    if (filters.classifications.length > 0) {
      const toBackend: Record<Classification, Linea['clasificacion']> = {
        Alta: 'ALTA',
        Moderada: 'MODERADA',
        Baja: 'BAJA',
      };
      const selected = new Set(filters.classifications.map((c) => toBackend[c]));
      filtered = filtered.filter((l) => selected.has(l.clasificacion));
    }

    if (filters.priority) {
      filtered = filtered.filter((l: Linea) => l.prioridad === filters.priority);
    }

    return filtered;
  }, [allLineas, focusedLineId, filters.classifications, filters.priority]);
  ...
  const fallas = useMemo(() => {
    ...
    if (filters.statuses.length > 0) {
      const toBackend: Record<FaultStatus, Falla['estado']> = {
        Abierta: 'ABIERTA',
        'En atención': 'EN_ATENCION',
        Cerrada: 'CERRADA',
      };
      const selected = new Set(filters.statuses.map((s) => toBackend[s]));
  ...
  const subestaciones = useMemo(() => {
    if (focusedLineId) return allSubestaciones.filter((s) => s.linea_id === focusedLineId);
    return allSubestaciones;
  }, [allSubestaciones, focusedLineId]);
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 h-[calc(100vh-8rem)]">
      <div className="w-full lg:w-80 flex flex-col gap-4 max-h-[40vh] lg:max-h-none">
