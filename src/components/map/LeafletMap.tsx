...
import { Estructura, Falla, Subestacion, parseGeometry } from '../../lib/supabase';

interface LeafletMapProps {
  estructuras: Estructura[];
  fallas: Falla[];
  subestaciones: Subestacion[];
  lineas: Array<{ id: string; numero: string; geom: unknown | null }>;
...
export default function LeafletMap({
  ...
  useEffect(() => {
    ...
    const filteredSubestaciones = focusedLineId
      ? subestaciones.filter((s) => s.linea_id === focusedLineId)
      : subestaciones;
    ...
    filteredLineas.forEach((linea) => {
      ...
      if (geom.type === 'MultiLineString') {
        geom.coordinates.forEach((segment: number[][]) => {
          const latLngs = segment.map((c: number[]) => [c[1], c[0]] as [number, number]);
          addPolyline(linea.id, linea.numero, latLngs);
        });
      }
    });
    ...
    // --- CLUSTER ESTRUCTURAS ---
    const estructurasCluster = L.markerClusterGroup({
    ...
    filteredSubestaciones.forEach((subestacion) => {
      const geom = parseGeometry(subestacion.geom);
      if (!geom || geom.type !== 'Point') return;
      ...
      const m = L.circleMarker([lat, lon], {
        ...
        opacity: 1,
        fillOpacity: 0.9,
      }).bindPopup(`<div><strong>${subestacion.nombre}</strong></div>`);

      subLayer.addLayer(m);
    });

    overlay.addLayer(subLayer);
    ...
    const fallasCluster = L.markerClusterGroup({
      showCoverageOnHover: false,
      maxClusterRadius: 50,
      iconCreateFunction: (cluster: L.MarkerCluster) => {
        const count = cluster.getChildCount();
  ...
  }, [estructuras, fallas, subestaciones, lineas, focusedLineId, onSelectEstructura, onSelectFalla, onSelectLinea]);
