...
export default function DetailPanel({ item, type, onClose, onCenterMap }: DetailPanelProps) {
  ...
  const estructura = isEstructura && item ? (item as Estructura) : null;
  const falla = !isEstructura && item ? (item as Falla) : null;
  const lineaId = estructura?.linea_id ?? falla?.linea_id ?? null;
